...
If the device is not managed, you cannot select a setup from the device. Status is indicated next to the
...
in
Devices Details.
Configure the size of the barcode being exported. The options are Small, Medium, or Large. The default size is Medium.
Enable
Replace Network Settings
to replace the network settings from the configuration file with the network settings of the new device onto which the configuration is being deployed.
Enable
Replace Hostname
