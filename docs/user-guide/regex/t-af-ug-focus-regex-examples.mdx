...
description: This section provides examples of common RegEx use cases and outlines the procedure to utilize them in Aurora Focus.
...
- `.` allows any sign
- `{11}` is the number of signs needed for the result to be true. All orange codes are shorter than 11 signs.
...
- `.` allows any sign.
- `{3,12}` the first number is the minimum number of signs needed to be true, and the second number represents the maximum.
...
- `.` allows any sign.
- `$78` is the identifier needs to be at the beginning of the string.
- `{3,12}` the first number is the minimum number of signs needed to be true, and the second number represents the maximum.
...
- `\d` allows numbers only (0-9).
- `{12}` number of signs needed to be true, and the second number represents the maximum. It's one less than the code length because the full string consists of the fixed first number + 12 numbers.
- `|` is the logical OR.
