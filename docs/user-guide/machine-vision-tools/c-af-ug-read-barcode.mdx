---
title: Read Barcode
description: Configure barcode decode settings such as Inverse 1D, ROI overlap, or
  ROI search.
...
Define the minimum percentage a barcode needs to be contained within the Region of Interest (ROI) to decode. For 1D barcodes, this applies only to the horizontal (X) dimension. For 2D barcodes, this applies to horizontal and vertical (X & Y) dimensions.
No Read String
...
2D DataMatrix BQM Parameters

Enable or disable parameter options to include in BQM calculations.


These settings are applicable to 2D DataMatrix barcodes only.
Minimum BQM Grade

Enable this option to use a threshold for issuing a pass or fail based on the BQM overall grade for each barcode scan.

For example, by setting the threshold between C and D, if a barcode achieves a BQM grade of A, B, or C, the job/tool issues a pass. If the overall grade is a D or an F, the BQM grade is a fail.
...
When enabled, the decoded data is output even if the BQM Overall Grade is a failing grade, based on the
Minimum BQM Grade
threshold.