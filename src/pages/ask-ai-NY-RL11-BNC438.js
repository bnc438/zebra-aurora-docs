...
const V2_SEARCH_EVENTS_STORAGE_KEY = 'askai.v2-search-events';
const MAX_V2_SEARCH_EVENTS = 500;
...
/**
 * Persists a V2 search-query-event (matches search-query-event.schema.json).
 * These are read by the DevDashboard V2 section from localStorage.
 */
function persistV2SearchQueryEvent(query, results, sessionId) {
  if (typeof window === 'undefined') return;
  try {
    const now = new Date();
    ...
    const event = {
      ...
      source: 'ask_ai',
      queryText,
      queryNormalized: normalizeQuery(queryText),
      zeroResultFlag: results.length === 0,
      resultCount: results.length,
      topClickedUrl: results[0]?.url || undefined,
      topicCandidate: results[0]?.category || undefined,
      sessionId: sessionId || getSessionId(),
      environment: 'prod',
    };
    persistAnalyticsEvent(V2_SEARCH_EVENTS_STORAGE_KEY, event, MAX_V2_SEARCH_EVENTS);
  } catch {
    // Never let telemetry break search UX.
  }
}
...
function trackSearch(query, results, confidence, meta) {
  ...
  persistV2SearchQueryEvent(query, results, searchEvent.sessionId);
