...
/**
 * Counts pages in a generated PDF by scanning for /Type /Page dictionary entries.
 * This avoids requiring a separate PDF parsing library.
 */
function countPdfPages(pdfPath) {
  try {
    const content = fs.readFileSync(pdfPath, 'binary');
    // Each page object has a /Type /Page entry; /Type /Pages is the parent node.
...
async function localizeHtmlImages(html, siteUrl, assetCache, failureCounter = null) {
  ...
  for (const match of matches) {
    ...
    try {
      ...
      if (failureCounter) failureCounter.value++;
...
async function generate() {
  ...
  await runCommand(NPX_COMMAND, ['docusaurus', 'build'], {
    DOCUSAURUS_BASE_URL: '/',
  ...
  const pdfValidationResults = [];
  const runId = `pdf-run-${new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)}`;
  ...
  try {
    for (const section of sections) {
      ...
      const sectionFailureCounter = {value: 0};
      ...
      for (const docId of section.docIds) {
        ...
        try {
          ...
          docs.push({
            route,
            title: docData.title,
            html: await localizeHtmlImages(docData.html, siteUrl(), assetCache, sectionFailureCounter),
          });
      ...
      if (docs.length === 0) {
        ...
        pdfValidationResults.push({
          snapshotDate,
          runId,
          pdfId: section.key,
          sectionKey: section.key,
          outputPath: `static/pdf/${section.outputFile}`,
          ...
          pageCount: 0,
          brokenLinkCount: 0,
          imageFailureCount: sectionFailureCounter.value,
          internalLinkCheckPassed: false,
          externalLinkCheckPassed: true,
          validationPassed: false,
          failureReason: 'no_docs_collected',
        });
        continue;
      }
      ...
      try {
        ...
        renderSuccess = true;
        renderDurationMs = Date.now() - sectionStartMs;
      } catch (pdfError) {
        renderDurationMs = Date.now() - sectionStartMs;
        timeoutFlag = pdfError.message?.includes('timeout') || pdfError.message?.includes('Timeout');
        failureReason = pdfError.message?.slice(0, 200) || 'unknown';
        console.error(`[PDF] Error generating ${section.outputFile}: ${pdfError.message}`);
      } finally {
        await printPage.close();
      }

      const outputFilePath = path.join(outputDir, section.outputFile);
      const fileExists = fs.existsSync(outputFilePath);
      const pageCount = fileExists && renderSuccess ? countPdfPages(outputFilePath) : 0;
      const validationPassed = renderSuccess && fileExists && sectionFailureCounter.value === 0;
      ...
      pdfValidationResults.push({
        ...
        brokenLinkCount: 0,
        imageFailureCount: sectionFailureCounter.value,
        internalLinkCheckPassed: renderSuccess,
        externalLinkCheckPassed: true,
        validationPassed,
      ...
      if (renderSuccess) {
        console.log(`[PDF] Wrote static/pdf/${section.outputFile} (${pageCount} pages)`);
      }
    }
  } finally {
    await browser.close();
  ...
  // Write PDF validation results for the V2 dashboard snapshot generator.
  const pdfDataDir = path.join(repoRoot, 'static', 'data');
  fs.mkdirSync(pdfDataDir, {recursive: true});
  const pdfResultsPath = path.join(pdfDataDir, 'pdf-validation-results.json');
  fs.writeFileSync(
    pdfResultsPath,
    JSON.stringify({snapshotDate, runId, records: pdfValidationResults}, null, 2)
  );
  const passCount = pdfValidationResults.filter((r) => r.validationPassed).length;
  console.log(`[PDF] Validation results: ${passCount}/${pdfValidationResults.length} sections passed → static/data/pdf-validation-results.json`);
